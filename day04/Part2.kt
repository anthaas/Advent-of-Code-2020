...
fun main(args: Array<String>) {
    ...
    val requiredFields = listOf(
        ...
        Pair("hgt", { x: String ->
            when {
                "cm" in x -> x.replace("cm", "").toIntOrNull() in 150..193
                "in" in x -> x.replace("in", "").toIntOrNull() in 59..76
                else -> false
            }
        }),
    ...
    val valid = passports.map { passport ->
        ...
        requiredFields.map { (value, validityCondition) -> it[value] != null && validityCondition(it[value]!!) }
            .all { it }
    }.count { it }
